...
A Tiny Tapeout **1x2** VGA graphics project written in Verilog. It displays four colored nearest-prototype regions at 640×480 and lets the user select the distance metric, move prototypes, or enable simple online training.
...
`ui_in[1:0]` selects one of four distance metrics:
...
The right side of the display shows the selected metric, training status, and prototype-color key. Metric and training changes become active at the next frame boundary.

## Controls

| Pin | Function |
|---|---|
| `ui_in[1:0]` | Distance-metric selection |
